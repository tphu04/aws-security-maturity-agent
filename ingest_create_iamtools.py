...
collection = client.get_or_create_collection(name="aws_sdk_docs_iam")
embeddings = OllamaEmbeddings(model="nomic-embed-text")

with open("services/iam/service-2.json", "r") as f:
    data = json.load(f)
...
for op_name, info in ops.items():
    ...
    collection.add(
        ids=[f"s3_{op_name}"],
        documents=[content],
        metadatas=[{"service": "iam", "action": op_name}]
    )
print(f"✅ Đã nạp {len(ops)} hàm iam vào thư viện kiến thức!")