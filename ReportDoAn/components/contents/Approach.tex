...
\begin{itemize}
  \item \textbf{Plan (Lập kế hoạch)}: \textbf{Planning Node} tiếp nhận yêu cầu từ người dùng và áp dụng kiến trúc \textit{Intent-first}: một lần gọi LLM bắt buộc để trích cấu trúc ý định (\texttt{mode}, \texttt{services}, \texttt{check\_ids}, \texttt{topics}), sau đó định tuyến tất định sang một trong ba nhánh --- xác thực Check ID, group scan, hoặc fan-out RAG hybrid retrieval theo từng cặp (service $\times$ topic) --- để xây dựng kế hoạch đánh giá chi tiết (Assessment Plan).
...
\input{components/diagrams/architecture_3layer}

Hình~\ref{fig:architecture_3layer} chi tiết hóa ranh giới tiến trình của hệ thống: Frontend (UI) giao tiếp HTTP/JSON với Orchestrator (stateful graph runtime); Orchestrator điều phối các module gọi vào Scanner CLI, RAG Service, và Local LLM Runtime; lớp dữ liệu gồm một relational store cho jobs/checkpoint và một vector index cho RAG.
...
Quy trình vận hành của Orchestration Layer được khởi tạo tại giai đoạn \textbf{Thu thập và Lập kế hoạch (Plan)}. Tại đây, node \texttt{environment} đóng vai trò tiền trạm, chịu trách nhiệm thiết lập các kết nối an toàn thông qua việc xác thực danh tính (IAM roles và AWS STS) để trích xuất ngữ cảnh của tài khoản AWS mục tiêu. Tại node \texttt{planning}, hệ thống thực hiện một lần gọi LLM duy nhất để phân loại ý định người dùng (\textit{intent classifier}); kết quả phân loại điều hướng tất định sang một trong ba nhánh, trong đó RAG (CIS AWS Foundations Benchmark và AWS Well-Architected Framework) được kích hoạt có điều kiện --- lexical lookup khi cần xác thực Check ID, hybrid retrieval khi cần ánh xạ \textit{topic} sang Check ID cụ thể --- để xây dựng phạm vi đánh giá (\textit{Assessment Plan}). Tiếp theo, hệ thống chuyển sang \textbf{Giai đoạn Thực thi (Do)}: node \texttt{scan\_submit} kích hoạt công cụ \textit{Prowler} thông qua Scanner API để quét các dịch vụ AWS bất đồng bộ; node \texttt{scan\_poll} hoạt động ở vòng lặp polling theo dõi tiến trình quét; và node \texttt{scan\_collect} tổng hợp các raw findings khi tất cả job hoàn tất.
...
\begin{table}[!htb]
  ...
  \begin{tabular}{|p{4.0cm}|l|p{7.5cm}|}
    ...
    Planner                 & Plan  & Sinh kế hoạch đánh giá (Intent-first) từ yêu cầu ngôn ngữ tự nhiên \\ \hline
...
\subsection{Hạ tầng và công cụ tích hợp}
...
\begin{itemize}
  \item \textbf{Dịch vụ LLM và Tri thức (Local LLM \& RAG):} 
    \begin{itemize}
      \item \textit{Local SLM Runtime:} Các module gọi LLM được cấu hình chạy một SLM ($\leq$ 4B tham số, lượng tử hóa Q4) trên phần cứng cục bộ thông qua một runtime tương thích chuẩn OpenAI Chat Completions. Sự cô lập này đảm bảo \textit{Data Privacy}: cấu hình nhạy cảm từ IAM/Network không bị lộ ra API bên thứ ba (OpenAI, Anthropic). Cụ thể model và lượng tử hóa được trình bày ở Chương~5.
...
\section{Thiết kế luồng điều phối trạng thái}
...
\subsection{Kiến trúc persistence và resume}
...
\input{components/diagrams/persistence_sequence}
...
Cấu hình Observability Layer (endpoint backend, API key, tỷ lệ sample) được tham số hóa; ở môi trường offline có thể disable hoàn toàn — toàn bộ helper trở thành no-op silent. Lib SaaS dùng để render trace UI được trình bày ở Chương~5.
...
Trong kiến trúc này, RAG Engine đóng vai trò là một dịch vụ trung gian (RAG-as-a-Service), kết nối giữa các module và nguồn tri thức. Các module không trực tiếp truy cập dữ liệu mà gửi yêu cầu đến RAG Engine thông qua các API nội bộ. Cách tổ chức này giúp chuẩn hóa toàn bộ quá trình truy hồi, đồng thời đảm bảo tính nhất quán và khả năng tái sử dụng giữa các module.
...
\begin{itemize}
    ...
    \item Xây dựng ngữ cảnh phù hợp với từng loại module
\end{itemize}
...
Trong quá trình vận hành, luồng xử lý tổng quát của hệ thống có thể được mô tả như sau: các module gửi truy vấn đến RAG Engine, hệ thống tiến hành phân tích và truy hồi dữ liệu từ các nguồn tri thức, sau đó xây dựng một tập ngữ cảnh có cấu trúc và trả về cho agent. Agent sử dụng ngữ cảnh này làm đầu vào cho quá trình suy luận hoặc sinh nội dung, tùy thuộc vào mục đích sử dụng.

Một điểm quan trọng trong thiết kế là ngữ cảnh không được xây dựng theo một khuôn mẫu chung, mà được điều chỉnh theo từng loại module. Ví dụ, Planning Module cần tập trung vào phạm vi kiểm tra và độ bao phủ, trong khi Risk Evaluation Module cần thông tin chi tiết về rủi ro và ánh xạ kiểm soát. Điều này cho thấy hệ thống không chỉ dừng lại ở mức truy hồi thông tin, mà còn thực hiện bước trung gian nhằm tối ưu hóa dữ liệu đầu vào cho từng tác vụ cụ thể.
...
Kiến trúc logic của hệ thống RAG được thiết kế theo mô hình phân tầng (Layered Architecture) gồm sáu lớp chức năng: \textbf{Knowledge Engineering} và \textbf{Storage} (xử lý ngoại tuyến); \textbf{Query} và \textbf{Retrieval Intelligence} (truy vấn thời gian thực); \textbf{Context Construction} và \textbf{Generation} (hậu xử lý). Việc phân tầng tách bạch trách nhiệm: quá trình nhúng dữ liệu và quản lý kho lưu trữ lai (BM25 và ChromaDB) diễn ra ngoại tuyến, độc lập với pha trực tuyến; trong pha trực tuyến, hệ thống định tuyến truy vấn, lọc ngữ cảnh và đóng gói ngữ cảnh riêng biệt cho từng Agent đích. Sơ đồ sau trực quan hóa toàn bộ luồng dữ liệu từ truy vấn đến đầu ra của tầng sinh:

\input{components/diagrams/rag_logic}
...
\begin{enumerate}
    ...
    \item \textbf{Agentic Generation (Sinh đầu ra có cấu trúc):} Đưa khối ngữ cảnh đã chọn lọc vào prompt template, mô hình LLM sinh đầu ra theo schema cố định (\textit{Structured JSON Output}) đã định nghĩa trước cho từng module, giúp downstream parsing không bị ảnh hưởng bởi format drift.
\end{enumerate}
...
\begin{lstlisting}[language=json, caption=Mẫu dữ liệu ánh xạ giữa kỹ thuật và quản trị]
{
  ...
  "mapping_reason": "Selected via 2-proposer consensus (pairwise top-1 agreement=1.0). Supported by framework references: aws_fsbp, cis_aws.",
  "status": "active",
  "review_status": "auto_high",
  "evidence_refs": [
    {"source": "aws_fsbp", "ref_type": "check_type", "raw": "AWS Foundational Security Best Practices"},
    {"source": "cis_aws", "ref_type": "check_type", "raw": "CIS AWS Foundations Benchmark"}
  ],
  "consensus": {
    "voters": ["lexical@v1", "tfidf_embedding@v1"],
    "rank1_per_voter": ["block_public_access", "block_public_access"],
    "pairwise_top1_agreement_rate": 1.0
  },
  "provenance": {
    "generated_by": "rebuild-pipeline@v2.0",
    "generated_at": "2026-05-17T..."
  },
  "lifecycle": {"status": "active", "next_review_due": "2026-11-13T..."},
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item Đọc dữ liệu thô đã được chuẩn hóa.
...
\subsection{Pipeline truy hồi lai (Hybrid Retrieval)}
...
\begin{table}[!htb]
  ...
  \begin{tabular}{|p{2.2cm}|p{4.6cm}|p{4.5cm}|}
    ...
    \textbf{(A) Translation pre-step} VI $\rightarrow$ EN ở Planning Module rồi giữ embedding EN-only (BGE-base-en, MS~MARCO cross-encoder) & Tận dụng tối đa mô hình EN đã pretrain trên corpus bảo mật EN; reranker EN-only hoạt động đồng nhất; pipeline đơn giản, chỉ một index & Thêm một bước chuẩn hóa thuật ngữ (deterministic VI$\rightarrow$EN glossary, không cần LLM) trước embedding; phụ thuộc độ phủ glossary \\ \hline
...
\subsection{Xây dựng ngữ cảnh và giao diện RAG-as-a-Service}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Hỗ trợ các module tổng hợp thông tin và đưa ra nhận định ở mức cao hơn
    \end{itemize}
    ...
    Mapping giữa check và capability được xây dựng qua một quy trình bán tự động gồm 4 tầng, được thiết kế nhằm không phụ thuộc vào một người kiểm duyệt cá nhân:
    \begin{itemize}
        \item \textbf{Tầng 1 -- Trích xuất tín hiệu từ nguồn có thẩm quyền:} với mỗi check, hệ thống phân tích các trường \texttt{CheckType}, \texttt{Categories}, \texttt{AdditionalURLs} trong dữ liệu Prowler để trích các tham chiếu framework chính thức (NIST 800-53, CIS AWS Foundations v3.0.0, AWS FSBP, PCI-DSS, HIPAA, ISO 27001). Trên 577 check, 84.2\% có thể suy ra \textit{security domain}, 48.5\% có ít nhất một \textit{evidence reference} đến framework công nhận.

        \item \textbf{Tầng 2 -- Đề xuất ứng viên độc lập (multi-proposer):} từ tín hiệu Tầng 1, các \textit{proposer} độc lập về không gian đặc trưng cùng đề xuất top-K capability ứng viên cho mỗi check. Hai proposer chính: (i) \texttt{lexical@v1} dựa trên Jaccard token, weighted overlap và intent rules; (ii) \texttt{tfidf\_embedding@v1} dựa trên TF--IDF cosine. Mỗi proposer chỉ \textit{đề xuất}, không tự quyết định trạng thái phê duyệt. Hệ thống có sẵn giao diện \texttt{LLMProposer} cho phép bổ sung voter thứ ba từ mô hình ngôn ngữ lớn.

        \item \textbf{Tầng 3 -- Đồng thuận và xếp hạng:} các đề xuất từ Tầng 2 được tổng hợp qua \textit{Borda count} và đo \textit{Cohen's kappa} cặp đôi giữa các proposer. Mỗi check được phân vào một trong bốn trạng thái: \texttt{consensus} (mọi proposer đồng thuận top-1), \texttt{majority}, \texttt{weak} (giao top-K khác rank-1), hoặc \texttt{disputed} (không có giao). Trên 577 check: 40.0\% đạt \texttt{consensus}, 45.4\% \texttt{weak}, và 14.6\% \texttt{disputed} được công khai trong \texttt{tier4\_excluded\_disputed.json} và \textbf{không} đưa vào artifact production. Cohen's kappa hiện tại giữa hai proposer là 0.38 (\textit{fair agreement} theo Landis--Koch); con số này có thể được nâng lên \textit{substantial} khi tích hợp voter LLM.

        \item \textbf{Tầng 4 -- Kiểm chứng định lượng:} một \textit{golden set} 49 mapping được xây dựng tay với trích dẫn cụ thể tới CIS v3.0.0, NIST 800-53 R5 và AWS FSBP. Tập golden được cross-verify bởi một mô hình ngôn ngữ ngoài hệ thống (ChatGPT, vai trò \textit{reviewer độc lập}) -- không trùng với bất kỳ proposer nào ở Tầng 2 -- nhằm tách biệt giữa người sinh và người duyệt. Audit trail chi tiết được lưu tại \texttt{external\_review\_audit.json}. Trên tập golden, độ chính xác top-1 trong nhóm \texttt{consensus} đạt 62.96\%, gấp năm lần so với nhóm \texttt{weak} (12.5\%) -- chứng minh tín hiệu đồng thuận có giá trị phân biệt thực sự.
    \end{itemize}

    Quy trình ghi artifact tuân theo chính sách \textit{promotion} chặt: chỉ những mapping ở trạng thái \texttt{consensus} mới được gán \texttt{status=active}, \texttt{review\_status=auto\_high} và lọt qua \texttt{filter\_for\_agent\_context} để các Agent downstream tiêu thụ. Mapping ở trạng thái \texttt{weak} được giữ ở \texttt{status=proposed} cho lần kiểm duyệt tiếp theo; mapping \texttt{disputed} bị loại hoàn toàn khỏi production.
    ...
    Để loại bỏ các liên kết sai khác domain (ví dụ một check về mã hóa S3 bị ánh xạ sang capability GenAI), hệ thống áp dụng ba họ ràng buộc bất biến chạy như test CI:
    \begin{itemize}
        \item \textbf{Reference integrity:} mọi \texttt{check\_id} và \texttt{capability\_id} trong artifact phải tồn tại trong catalog tương ứng.
        \item \textbf{Domain alignment:} security\_domain của check phải trùng security\_domain của capability (cho phép trường hợp một bên không suy ra được, ưu tiên không chặn nhầm hơn là chặn quá tay).
        \item \textbf{Banned entity gates:} các cặp pattern check--capability cấm tường minh dựa trên kinh nghiệm từ phiên bản cũ (ví dụ check \texttt{s3\_*encryption*} không được map sang capability \texttt{*bedrock|genai*}).
    \end{itemize}

    Ngoài ba họ trên, hệ thống còn có:
    \begin{itemize}
        \item \textbf{Threshold floors trong CI:} 9 cổng kiểm tra (\texttt{tests/mapping\_pipeline/test\_pipeline\_gates.py}) đặt ngưỡng tối thiểu cho coverage Tầng 1, tỷ lệ consensus, Cohen's kappa, độ chính xác trên golden set và khả năng tải qua \texttt{MappingService}. Nếu một pull request làm rớt bất kỳ ngưỡng nào, artifact sẽ không được hợp nhất.
        \item \textbf{Tính tái lập:} toàn bộ pipeline có thể chạy lại từ đầu (\texttt{python -m RAG.pipeline.mapping.*}). Mỗi mapping mang khối \texttt{provenance} ghi pipeline version, timestamp; khối \texttt{consensus} ghi danh sách voter và tỷ lệ đồng thuận; khối \texttt{lifecycle} ghi ngày cần kiểm duyệt lại.
    \end{itemize}

    Kết quả: trên artifact hiện tại, cả ba họ ràng buộc bất biến đều không phát hiện vi phạm (0 violation trên 493 mapping được xuất).
    ...
    \begin{itemize}
        ...
        \item Cung cấp ngữ cảnh ở mức khái niệm cho các module
    \end{itemize}
...
Context preparation là một bước quan trọng nhưng thường bị bỏ qua trong các hệ thống RAG cơ bản. Việc tách riêng bước lựa chọn và làm sạch dữ liệu giúp kiểm soát tốt hơn chất lượng ngữ cảnh đầu ra. Điều này đặc biệt quan trọng trong bối cảnh hệ thống phục vụ nhiều loại module khác nhau, mỗi loại có yêu cầu riêng về thông tin. Bằng cách giảm nhiễu và chuẩn hóa dữ liệu, hệ thống tạo ra một nền tảng ổn định cho bước tiếp theo là xây dựng ngữ cảnh theo từng mục tiêu sử dụng.



\subsubsection*{Context Bundling theo Agent (Planning / Risk / Report)}

Sau khi dữ liệu đã được lựa chọn và làm sạch, bước tiếp theo là tổ chức lại thông tin thành các cấu trúc ngữ cảnh phù hợp với từng loại module. Trong hệ thống, thay vì sử dụng một định dạng ngữ cảnh chung cho mọi trường hợp, dữ liệu được đóng gói (\textit{bundle}) theo mục tiêu sử dụng cụ thể. Đây là một điểm khác biệt quan trọng so với các hệ thống RAG truyền thống.
...
\begin{enumerate}
    ...
    Do đó, hệ thống áp dụng cách tiếp cận \textit{consumer-aware context}, trong đó ngữ cảnh được thiết kế riêng cho từng loại module.
    ...
    Context bundling là bước chuyển đổi từ dữ liệu truy hồi sang dữ liệu phục vụ suy luận. Việc thiết kế ngữ cảnh theo từng loại module mang lại các lợi ích:
    \begin{itemize}
...
Việc xây dựng context theo từng loại module thể hiện một bước tiến so với các hệ thống RAG cơ bản. Thay vì coi truy hồi là mục tiêu cuối cùng, hệ thống xem truy hồi là bước trung gian và tập trung vào việc tổ chức lại tri thức theo mục tiêu sử dụng. Cách tiếp cận này không chỉ nâng cao hiệu quả của hệ thống mà còn tạo nền tảng mở rộng, cho phép bổ sung các loại module mới mà không cần thay đổi pipeline truy hồi.

\subsubsection*{API và tích hợp hệ thống}

Sau khi xây dựng hoàn chỉnh pipeline truy hồi và cơ chế tổ chức ngữ cảnh, hệ thống RAG được triển khai dưới dạng một dịch vụ độc lập và tích hợp với các module thông qua các API nội bộ. Phần này trình bày cách thiết kế giao diện lập trình ứng dụng (API) và cơ chế tương tác giữa RAG Engine và các thành phần khác trong hệ thống.

\begin{enumerate}
    \item \textbf{Thiết kế RAG như một dịch vụ}

    RAG Engine được triển khai theo kiến trúc hướng dịch vụ (\textit{service-oriented architecture}), cung cấp các endpoint cho phép các module gửi yêu cầu truy hồi và nhận về ngữ cảnh đã được xử lý.

    Việc tách RAG thành một service riêng biệt mang lại các lợi ích:
    \begin{itemize}
        \item Tăng tính mô-đun, giúp các module không cần quan tâm đến chi tiết của quá trình truy hồi
        \item Cho phép nâng cấp độc lập các thành phần như embedding model, chiến lược retrieval hoặc nguồn dữ liệu
        \item Hỗ trợ mở rộng hệ thống trong các kịch bản nhiều module sử dụng chung một nguồn tri thức
    \end{itemize}

    Cách tiếp cận này giúp tách biệt rõ ràng giữa lớp truy hồi tri thức và lớp suy luận của các module.
    ...
    Trong quá trình vận hành, các module tương tác với RAG Engine theo một luồng xử lý thống nhất. Thay vì truy cập trực tiếp vào dữ liệu, module gửi yêu cầu thông qua API và nhận về kết quả đã được xử lý.

    Luồng tương tác bao gồm:
    \begin{itemize}
        \item Agent xác định nhu cầu (planning, risk analysis hoặc reporting)
        \item Module gửi yêu cầu đến RAG Engine kèm theo các tham số cần thiết
    ...
    Việc sử dụng dữ liệu có cấu trúc giúp các module dễ dàng xử lý và tái sử dụng thông tin mà không cần thực hiện thêm các bước chuyển đổi phức tạp.
    ...
    Ví dụ, Planning Module có thể gửi truy vấn tổng quát để nhận danh sách các kiểm tra cần thực hiện, trong khi Risk Evaluation Module gửi thông tin về một finding cụ thể để nhận ngữ cảnh phân tích chi tiết. Mặc dù sử dụng cùng một endpoint, kết quả trả về sẽ khác nhau nhờ cơ chế context bundling.

    Cách tích hợp này mang lại các lợi ích:
    \begin{itemize}
        \item Giảm trùng lặp logic giữa các module
...
\subsection{Mẫu thiết kế Constrained Tool Invocation}
\label{subsec:constrained_tool_pattern}

Ba module gọi LLM trong hệ thống (Planning, Risk Evaluation, Remediation Planner) cùng chia sẻ một pattern thiết kế chung mà chúng em gọi là \textbf{Constrained Tool Invocation Pattern}. Pattern này giải quyết bài toán: \textit{LLM mạnh ở việc hiểu ngôn ngữ tự nhiên nhưng yếu khi điền tham số kỹ thuật chính xác (resource ID, region, account ID, CIDR\ldots)}.
...
\begin{table}[!htb]
  ...
  \begin{tabular}{|l|p{4.0cm}|p{4.0cm}|}
    ...
    Planning (\S\ref{subsec:planning_module}) & Intent classification (mandatory, single call) & Lexical ID validation; service whitelist gating; weighted scoring; fan-out retrieval per (service $\times$ topic) \\ \hline
...
Lợi ích pattern hóa: (i) tái sử dụng cùng pattern ở 3 module thay vì design lại; (ii) audit dễ — mọi sai lệch đều có một trong hai nguồn (LLM mis-classify hoặc code param mis-bind), không lẫn lộn; (iii) khi đo lường, có thể ablation từng phần riêng để cô lập đóng góp của LLM-side và code-side.

\section{Ví dụ minh hoạ end-to-end: S3 Public Access}
...
\begin{enumerate}
  ...
  Planning Module: \textsc{LLMClassifyIntent} trả về JSON $\{\texttt{mode}=\textit{specific\_checks},\; \texttt{services}=[\text{s3}],\; \texttt{check\_ids}=[],\; \texttt{topics}=[\textit{public access}]\}$ (vì câu chứa cụm ``public access'' --- một topic bảo mật cụ thể). Router rẽ vào nhánh \textit{specific\_checks}: fan-out RAG hybrid retrieval cho cặp (\textit{s3}, \textit{public access}), chấm bằng công thức trọng số, lấy top ứng viên qua trần $\text{TOP\_K\_PER\_SUBQUERY}=3$. Kết quả: \texttt{assessment\_plan} = \{services: [s3], specific\_checks: [s3\_account\_level\_public\_access\_blocks, s3\_bucket\_policy\_no\_public, s3\_public\_access\_block]\}.
...
Ví dụ này minh hoạ ba điểm thiết kế quan trọng: (i) Planning chỉ tốn một lần gọi LLM ở đầu pipeline để trích intent, mọi định tuyến và truy hồi sau đó đều tất định --- minh chứng cho RQ5 (local-LLM feasibility); (ii) Two-Pass + RAG enrich cấp ngữ cảnh đủ để LLM giải thích chính xác trong reasoning --- RQ2; (iii) Maturity delta phản ánh chính xác state change, không chỉ là ``count FAIL giảm 1'' --- RQ1 và sự khác biệt với baseline CSPM (RQ5).
...
\subsubsection*{Planning Module: Kiến trúc Lập kế hoạch Intent-first}
...
\textbf{Chiến lược thiết kế --- Intent-first, RAG-conditional.} Thiết kế đặt một lần gọi LLM \textit{duy nhất và bắt buộc} ở đầu pipeline để trích cấu trúc ý định (\textit{intent}) của yêu cầu, sau đó toàn bộ định tuyến và truy hồi đều tất định. LLM chỉ trả về một JSON schema khép kín gồm bốn trường --- \texttt{mode}, \texttt{services}, \texttt{check\_ids}, \texttt{topics} --- và \textit{không} được phép chọn check ID từ một bể ứng viên. Điều này triệt hạ rủi ro LLM bịa Check ID (\textit{ID hallucination}) bằng thiết kế kiến trúc thay vì dựa vào hậu kiểm. RAG được kích hoạt sau, có điều kiện: lexical lookup khi cần xác thực ID người dùng nhập; hybrid retrieval khi cần ánh xạ \textit{topic} thành Check ID cụ thể; không được gọi cho nhánh \textit{group scan}.

\subsubsection*{Pipeline xử lý (4 giai đoạn)}

\begin{enumerate}
  \item \textbf{LLM Intent Classification (mandatory).} Một lần gọi LLM với \texttt{temperature=0} và \texttt{format=json} áp prompt phân loại có bảy quy tắc cứng (R1--R7) và một chuỗi ví dụ ít-shot. Đầu ra là JSON\
    $\{\texttt{mode} \in \{\textit{group\_scan}, \textit{specific\_checks}\},\;
       \texttt{services} \subseteq \mathcal{S}_{valid},\;
       \texttt{check\_ids},\;
       \texttt{topics}\}$.\
    Hai bất biến quan trọng do prompt áp đặt: (i) \textit{extract, do not infer} --- chỉ liệt kê service được nêu đích danh hoặc qua từ khóa 1-1 (\textit{bucket}$\to$s3, \textit{cluster}$\to$eks, ...), cấm suy đoán service liên quan; (ii) \textit{action verbs are not topics} --- \textit{``kiểm tra iam''} là \textit{group\_scan} với rỗng topic, không phải \textit{specific\_checks}.
  \item \textbf{Deterministic Router.} Dựa vào trường \texttt{mode} và sự tồn tại của \texttt{check\_ids}, router rẽ vào một trong ba nhánh tất định:
    \begin{itemize}
      \item \textit{Explicit check IDs} $\to$ xác thực lexical từng ID qua RAG (chế độ \texttt{retrieval\_mode=lexical}, không cần embedding); ID hợp lệ trả thẳng (fast-track), ID không hợp lệ được dùng làm \textit{hint} cho retrieval.
      \item \textit{group\_scan} với services hợp lệ $\to$ phát \texttt{groups\_to\_scan} ngay, \textit{không} gọi RAG.
      \item \textit{specific\_checks} $\to$ fan-out RAG hybrid retrieval cho từng tích Descartes $(\text{service} \times \text{topic})$; mỗi sub-query lấy tối đa $\text{TOP\_K\_PER\_SUBQUERY}=3$ ứng viên sau khi qua bước chấm điểm; bể gộp có trần $\text{MAX\_MERGED\_CANDIDATES}=10$.
    \end{itemize}
  \item \textbf{Deterministic Scoring.} Mỗi ứng viên từ RAG được chấm bằng hàm tổng có trọng số\
    $\text{score} = 0{,}6 \cdot \text{sim}_{RAG} + 0{,}3 \cdot w_{sev} + 0{,}1 \cdot \mathbb{1}_{\text{service\_match}}$,\
    trong đó $w_{sev}$ tra từ bảng trọng số mức nghiêm trọng (\textit{critical}=1{,}0, \textit{high}=0{,}8, \textit{medium}=0{,}5, \textit{low}=0{,}2). Bước này thuần tất định, không gọi LLM.
  \item \textbf{Whitelist gating.} Tập service đầu ra bị giới hạn về whitelist chín nhóm hỗ trợ scan trực tiếp $\mathcal{S}_{ALLOWED} = \{\text{s3, iam, ec2, rds, cloudtrail, eks, vpc, lambda, kms}\}$; service ngoài whitelist bị loại khỏi plan và ghi log nhưng không làm fail pipeline.
\end{enumerate}
...
\begin{algorithmic}[1]
  ...
  \State $\text{intent} \gets \textsc{LLMClassifyIntent}(q)$ \Comment{1 LLM call, JSON-constrained}
  \State $S \gets \text{intent.services} \cap \mathcal{S}_{ALLOWED}$
  \If{$\text{intent.check\_ids} \neq \emptyset$}
    \State $(V, I) \gets \textsc{LexicalValidate}(\text{intent.check\_ids})$ \Comment{RAG lexical lookup}
    \If{$V \neq \emptyset \land I = \emptyset$}
      \State \Return \textsc{FastTrack}(V) \Comment{all IDs verified, no further retrieval}
    \EndIf
    \State $\text{intent.topics} \gets \text{intent.topics} \cup V$ \Comment{seed retrieval with valid IDs}
  \EndIf
  \If{$\text{intent.mode} = \textit{group\_scan}$}
    \State \Return \textsc{EmitGroups}(S) \Comment{no RAG}
  \EndIf
  \State $C \gets \emptyset$ \Comment{specific\_checks: fan-out per (service, topic)}
  \ForAll{$(s, t) \in S \times \text{intent.topics}$}
    \State $\text{cand} \gets \textsc{HybridRetrieve}(s \;\Vert\; t)$
    \State $\text{scored} \gets \textsc{Score}(\text{cand}, s)$ \Comment{Eq. weighted-sum above}
    \State $C \gets C \cup \textsc{Top}(\text{scored},\; \text{TOP\_K\_PER\_SUBQUERY})$
    \If{$|C| \geq \text{MAX\_MERGED\_CANDIDATES}$} \State \textbf{break} \EndIf
  \EndFor
  \State \Return \textsc{EmitChecks}(C)
\end{algorithmic}

\textbf{Hệ quả thiết kế.} Một lần gọi LLM duy nhất ở đầu pipeline; mọi bước còn lại đều tất định. So với pattern \textit{LLM-in-the-loop} truyền thống (LLM xen kẽ ở nhiều bước), thiết kế này có ba tính chất: (i) \textit{predictable cost} --- chi phí token và độ trễ LLM trên mỗi request là hằng số, không phụ thuộc vào độ phức tạp truy vấn; (ii) \textit{no ID fabrication} --- LLM không được phép chọn từ bể ứng viên, do đó không thể bịa Check ID không tồn tại; (iii) \textit{multi-service native} --- yêu cầu kiểu ``scan s3 iam ec2 rds'' được xử lý đồng nhất qua trường \texttt{services} là một list, không cần regex tách câu. Đây là cơ sở giúp Planning Module khả thi trên phần cứng cá nhân tầm trung (RQ5).
...
\begin{itemize}
  ...
  \item \textbf{Thu thập và Chuẩn hóa:} khi status chuyển sang \texttt{COMPLETED}, kết quả được tải về theo định dạng OCSF (Open Cybersecurity Schema Framework). Một lớp lọc giữ lại các trường cốt lõi (\texttt{Resource ID}, \texttt{Region}, \texttt{Status}, \texttt{Severity}) trước khi chuyển sang Risk Evaluation.
\end{itemize}
...
\textbf{Risk Evaluation Module} là lớp phân tích trung tâm, chịu trách nhiệm chuyển đổi raw findings thành thông tin rủi ro đã định lượng và chuẩn hóa. Thiết kế tập trung vào hai mục tiêu: \textit{Noise Reduction} (giảm nhiễu trước khi tiêu thụ ngữ cảnh LLM) và \textit{Prioritization} (xếp hạng theo ngữ cảnh thực tế).

\subsubsection*{Chiến lược Sàng lọc và Tối ưu hóa Ngữ cảnh}
...
\textbf{Lưu ý disambiguate ngữ nghĩa.} Trong báo cáo này, hai khái niệm \textit{confidence} thường bị nhầm lẫn được tách nhãn rõ:
\begin{itemize}
...
\begin{table}[!htb]
  ...
  \begin{tabular}{|p{1.8cm}|p{4.0cm}|p{3.0cm}|p{4.0cm}|}
    ...
    RQ5 --- Local-LLM feasibility & SLM $\leq$ 4B + Q4; Intent-first single-LLM-call planning + tất định-only routing (\S\ref{subsec:planning_module}) & \S{}6.2 hardware footprint; \S{}6.x latency \& cost & Median latency $\leq 30\text{s}$/decision; mỗi planning request tiêu đúng một lần gọi LLM; chạy trên GPU 6GB VRAM \\ \hline
  \end{tabular}
\end{table}
