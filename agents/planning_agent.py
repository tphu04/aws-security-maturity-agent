...
COMMON_STOP_WORDS = [
    # Hành động
    "scan", "check", "quét", "kiểm", "tra", "xem", "verify", "audit", "assess",
    # Số lượng / Phạm vi
    "all", "tất", "cả", "full", "toàn", "bộ", "mọi", "hết", "các", "những", "cái",
    # Đối tượng chung
    "group", "nhóm", "dịch", "vụ", "service", "services", "aws", "cloud",
    # Liên từ / Giới từ
    "liên", "quan", "tới", "đến", "về", "related", "to", "in", "on", "for", "of", "with",
    "cho", "tôi", "dùm", "hộ", "là", "bị", "có", "have", "has", "the"
]
...
class PlanningAgent(BaseAgent):
    ...
    def _semantic_search(self, queries: List[str]) -> List[str]:
        ...
        refined_queries = []
        for q in queries:
            clean_q = q.lower().replace(" và ", ",").replace(" and ", ",").replace(" với ", ",").replace(" with ", ",")
            parts = clean_q.split(",")
            for p in parts:
                if p.strip():
        ...
        for query in refined_queries:
            ...
            if strict_matches:
                print(f"     -> Query '{query}': Found {len(strict_matches)} STRICT matches.")
    ...
    def run(self, user_request: str) -> Dict[str, Any]:
        ...
        alias_map = {"storage": "s3", "lưu trữ": "s3", "compute": "ec2", "danh tính": "iam", "tài khoản": "iam"}
        ...
        if detected_service:
            ...
            for w in words:
                clean_w = w.strip(",.?!")
                if clean_w == detected_service: continue # Bỏ tên service
                if clean_w in COMMON_STOP_WORDS: continue # Bỏ từ rác
                if clean_w in alias_map: continue # Bỏ alias
                meaningful_words.append(clean_w)
            
            # 1.3 QUYẾT ĐỊNH
            if not meaningful_words:
                # Không còn từ nào có nghĩa -> GROUP SCAN
                print(f"   [Rule Override] Pure intent detected: 'GROUP SCAN' for {detected_service}")
                return {"groups_to_scan": [detected_service], "checks_to_scan": [], "reasoning": "Pure group scan intent"}
            else:
                # Vẫn còn từ (VD: "public", "encryption") -> SPECIFIC SCAN (Để Semantic Search xử lý)
                print(f"   [PlanningAgent] Specific intent detected (Keywords: {meaningful_words}). Skipping Rule Override.")
        ...
        try:
            raw_output = (prompt | self.llm | StrOutputParser()).invoke({
                ...
                "allowed_groups": ", ".join(top_groups)
            })
            ...
            # Fallback Splitter
            if not queries or (len(queries) == 1 and (" và " in user_request or "," in user_request)):
                print("   [PlanningAgent] AI missed split. Using Python Fallback.")
                raw_splits = user_request.replace(" và ", ",").replace(" với ", ",").split(",")
                for s in raw_splits:
                    clean_s = s.strip()
            ...
            if is_group and target_service and not queries:
                ...
                return {"groups_to_scan": [svc], "checks_to_scan": [], "reasoning": f"AI identified group scan"}
                    ...
                    for q in queries:
                        for word in q.split():
                            if word in ALLOWED_GROUPS_LIST:
                                return {"groups_to_scan": [word], "checks_to_scan": [], "reasoning": "Fallback to group"}

            return {"groups_to_scan": ["iam"], "checks_to_scan": [], "reasoning": "Fallback default"}

        except Exception as e:
            print(f"   [PlanningAgent] Error: {e}")
            import traceback; traceback.print_exc()
            return {"groups_to_scan": ["s3"], "checks_to_scan": [], "reasoning": "Error fallback"}