...
def get_report(run_id: str, format: str = "markdown") -> Any:
    """Return the run's report. format=markdown (default) returns text/markdown
    suitable for download; format=json returns the structured sections list."""
    ...
    # Prefer the file produced by ReportAgent if it exists; otherwise rebuild
    # from the structured sections we have in memory.
    ...
    if content is None:
        parts: List[str] = [
            f"# PDCA Prowler Agent — Report\n",
            f"_Run: `{run_id}` · Generated: {report.get('generatedAt','')}_\n",
        ]
        for s in sections:
            parts.append(f"\n## {s['title']}\n\n{s['body']}\n")
        content = "\n".join(parts)
