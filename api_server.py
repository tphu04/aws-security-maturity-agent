...
ALLOWED_GROUPS = {
    ...
    "appstream", "appsync", "athena", "autoscaling", "awslambda", "backup",
    "bedrock", "cloudformation", "cloudfront", "cloudtrail", "cloudwatch",
    "codeartifact", "codebuild", "cognito", "config", "datasync", 
    "directconnect", "directoryservice", "dlm", "dms", "documentdb", "drs",
    "dynamodb", "ec2", "ecr", "ecs", "efs", "eks", "elasticache",
    "elasticbeanstalk", "elb", "elbv2", "emr", "eventbridge", "firehose",
    "fms", "fsx", "glacier", "glue", "guardduty", "iam", "inspector2",
    "kafka", "kinesis", "kms", "lightsail", "macie", "memorydb", "mq",
    "neptune", "networkfirewall", "opensearch", "organizations", "rds",
    "redshift", "resourceexplorer2", "route53", "s3", "sagemaker",
    "secretsmanager", "securityhub", "servicecatalog", "ses", "shield",
    "sns", "sqs", "ssm", "ssmincidents", "stepfunctions", "storagegateway",
    "transfer", "trustedadvisor", "vpc", "waf", "wafv2", "wellarchitected",
    "workspaces"
}

# (PROJECT_ROOT, JOB_OUTPUT_DIR... giữ nguyên)
PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
JOB_OUTPUT_DIR = os.path.join(PROJECT_ROOT, "job_outputs")
...
def _run_prowler_command_worker(job_id: str):
            ...
            try:
                ...
                with open(full_output_path, 'r', encoding='utf-8') as f:
                    ...
                    if entire_file_content:
                        ...
                        # Đảm bảo nó là list (mặc dù Prowler OCSF luôn là list)
                ...
                print(f"   -> Nội dung file (1000 ký tự đầu): {entire_file_content[:1000]}")
            except Exception as e:
                print(f"Job {job_id}: Lỗi khi đọc file OCSF (Exception): {e}")
        else:
             print(f"Job {job_id}: Lỗi không tìm thấy file output tại: {full_output_path}")
        # --- (HẾT SỬA LỖI) ---

        # (Bước 5: Cập nhật job... giữ nguyên)
    ...
    # (Phần 'except' (xử lý lỗi) giữ nguyên)
        ...
        # (Bước 6: Dọn dẹp file... giữ nguyên)
        if os.path.exists(full_output_path):
            try:
                os.remove(full_output_path)
                print(f"Job {job_id}: Đã dọn dẹp file {full_output_path}")
            except Exception as e:
...
def run_simple_scan(group: str, tasks: BackgroundTasks):
    ...
    MOCK_JOB_DATABASE[job_id] = {
        ...
        "task_type": "group", "task_value": group,
...
def run_custom_scan(filename: str, tasks: BackgroundTasks):
    ...
    if not os.path.isfile(full_path):
        raise HTTPException(status_code=404, detail=f"Không tìm thấy file check: {full_path}")
    try:
        with open(full_path, 'r') as f:
            checks_list = json.load(f)
        if not isinstance(checks_list, list) or not checks_list:
            raise HTTPException(status_code=400, detail="File JSON phải là một list và không rỗng.")
    ...
    MOCK_JOB_DATABASE[job_id] = {
        ...
        "submitted_time": time.time(),
        "task_type": "custom_file", "task_value": checks_string,
        "result": None, "error": None
    }
    tasks.add_task(_run_prowler_command_worker, job_id)
